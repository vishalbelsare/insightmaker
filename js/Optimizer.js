			...
			},
			{
				...
				handler: function () {
					...
					if (changePrimitives.length < 1) {
						showNotification(getText("You must select one or more primitives to adjust."), "error", true);
						return;
					}
					if (!goalPrimitive) {
						showNotification(getText("You must select a goal primitive to optimize."), "error", true);
						return;
					}
					var mySetting = getSetting();
...
function evaluateCurrentPoint() {
	...
	if (optimizerController.goalType == "Final Value") {
		...
		alert("Unknown optimizer goal type: " + optimizerController.goalType);
	}
